...
public class GetJavagames extends GenericAction 
{
	...
	public MBOutcome execute(MBDocument document, String path, String outcomeName) 
	{
	    ...
	    Utilities.setRequestParameter("Nokia6300", "Nokia6300", requestDoc);

	    MBDocument responseDoc = loadDocument("javagames", requestDoc);
	    MCDSUtilities.setPositions(responseDoc, "javagame", 0+((index-1)*9));
	    MCDSUtilities.resizeImages(responseDoc, "javagame", 80, 80);
	    
	    MBOutcome outcome = new MBOutcome("OUTCOME-page_javagames", responseDoc);
