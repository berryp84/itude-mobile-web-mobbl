...
public abstract class InitialController {
	...
	public void setInitialView() {
		_view.clear();
		MBOutcome outcome = new MBOutcome("OUTCOME-page_home",
				_dataManagerService.loadDocument("MBEmptyDoc"));
		handleOutcome(outcome);
	}

	public void initializeTab() {
		String outcomeName = null;
		if ("SHARES".equals(_view.getCurrentDialog()))
			outcomeName = "OUTCOME-tab_shares";
		else if ("SEARCH".equals(_view.getCurrentDialog()))
			outcomeName = "OUTCOME-SearchAction";
		else if ("ACCOUNT".equals(_view.getCurrentDialog()))
			outcomeName = "OUTCOME-tab_my_account";
		else if ("HOME".equals(_view.getCurrentDialog()))
			outcomeName = "OUTCOME-tab_home";
		else if ("INFO".equals(_view.getCurrentDialog()))
			outcomeName = "OUTCOME-page_info";

		if (outcomeName != null) {
			MBOutcome outcome = new MBOutcome(outcomeName,
					_dataManagerService.loadDocument("MBEmptyDoc"));
			handleOutcome(outcome);
		}
	}
	...
	public void handleOutcome(String outcomeName) {
		MBOutcome outcome = new MBOutcome(outcomeName, _view.getView()
				.getDocument());
	...
	public void doHandleOutcome(MBOutcome initialOutcome) {
		...
		List<MBOutcomeDefinition> definitions = _metadataService
				.getOutcomeDefinitionsForOrigin(initialOutcome.getOriginName(),
						initialOutcome.getOutcomeName());
		if (definitions.isEmpty())
			throw new MBNoOutcomesDefinedException("No outcome found for view "
					+ initialOutcome.getOriginName() + " with outcome "
					+ initialOutcome.getOutcomeName());
		...
		for (MBOutcomeDefinition def : definitions) {
			...
			outcomeToProcess.setNoBackgroundProcessing(initialOutcome
					.getNoBackgroundProcessing()
					|| def.getNoBackgroundProcessing());
			...
			if (outcomeToProcess.isPreConditionValid())
				outcome = outcomeToProcess;

		}

		if ("POPUP".equals(outcome.getDisplayMode()))
			_view.pushView(null);
		else if ("POP".equals(outcome.getDisplayMode()))
			_view.popView();
		else if ("MODAL".equals(outcome.getDisplayMode()))
			_view.pushView(null);
		else if ("ENDMODAL".equals(outcome.getDisplayMode()))
			_view.popView();

		// find out if the outcome's action corresponds to an action object
		Instance<MBAction> refined = _actions.select(new NamedQualifier(outcome
				.getAction()));
		if (!refined.isUnsatisfied()) {
			MBOutcome next = refined.get().execute(outcome.getDocument(),
					outcome.getPath(), outcome.getOutcomeName());
			if (next != null) {
			...
			MBViewController controller = getControllerForPage(outcome
					.getAction());

			if (outcome.getTransferDocument())
				controller.setDocument(outcome.getDocument());
			else
			...
			if (MBPageType.MBPageTypesErrorPage.equals(controller.getPage()
					.getPageType()))
				showAlertView(controller.getPage());
			else
				_view.replaceView(controller);
		}
	}
	...
	private void showAlertView(MBPage page) {
		...
		if (page.getDocument().getName().equals("MBException")) {
			...
			message = page.getDocument().getValueForPath(
					"/Exception[0]/@description");
		} else {
			title = page.getTitle();
			message = MBLocalizationService.getInstance().getTextForKey(
					(String) page.getDocument().getValueForPath(
							"/message[0]/@text"));
		}
	...
	private void handleException(MBOutcome outcome, Exception e) {
		...
		if (e instanceof MBException)
			name = ((MBException) e).getName();
		else
			name = e.getClass().getSimpleName();

		MBDocument exceptionDocument = _dataManagerService
				.loadDocument("MBException");
		exceptionDocument.setValue(MBLocalizationService.getInstance()
				.getTextForKey(name), "/Exception[0]/@name");
		exceptionDocument.setValue(MBLocalizationService.getInstance()
				.getTextForKey(e.getMessage()), "/Exception[0]/@description");
		exceptionDocument.setValue(outcome.getOriginName(),
				"/Exception[0]/@origin");
		exceptionDocument.setValue(outcome.getOutcomeName(),
				"/Exception[0]/@outcome");
		for (StackTraceElement ste : e.getStackTrace()) {
			String line = ste.toString();
			MBElement stackLine = exceptionDocument
					.createElementWithName("/Exception[0]/Stackline");
		...
		List<MBOutcomeDefinition> outcomeDefinitions = _metadataService
				.getOutcomeDefinitionsForOrigin(outcome.getOriginName(), name,
						false);
		if (!outcomeDefinitions.isEmpty()) {
			...
			outcomeDefinitions = _metadataService
					.getOutcomeDefinitionsForOrigin(outcome.getOriginName(),
							"exception", false);
			if (outcomeDefinitions.isEmpty()) {
				_log.warn("No outcome with origin="
			...
			MBOutcome genericExceptionHandler = new MBOutcome("exception",
					exceptionDocument);
	...
	private MBViewController getControllerForPage(String name) {
		Instance<MBViewController> refined = _viewControllers
				.select(new PageQualifier(name));
		if (!refined.isUnsatisfied())
			return refined.get();
		else {
