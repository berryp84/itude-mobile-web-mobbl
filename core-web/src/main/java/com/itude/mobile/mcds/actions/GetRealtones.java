...
public class GetRealtones extends GenericAction 
{
	...
	public MBOutcome execute(MBDocument document, String path, String outcomeName) {
	    ...
	    MBDocument responseDoc = loadDocument("realtones", requestDoc);
	    MCDSUtilities.setPositions(responseDoc, "realtone", 0+((index-1)*20));
	    
	    MBOutcome outcome = new MBOutcome("OUTCOME-page_realtones", responseDoc);
